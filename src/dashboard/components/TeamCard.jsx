/**
 * TeamCard.jsx — Team Operations Card
 *
 * Displays a team with:
 * - Team Name
 * - Vehicle
 * - Driver
 * - Supervisor
 ...
 *
 * Status colors:
 * - Green → active (fully assigned)
 * - Yellow → warning (partially assigned)
 * - Gray → unassigned
 */
...
/**
 * Get status indicator color class based on team status
 */
...
export const TeamCard = React.memo(({ team, isNight }) => {
    const { name, vehicleNumber, driver, supervisor, helper, route, isBackup, status } = team;
        ...
        >
            ...
            <div className="p-2 md:p-3 border-t-2 md:border-t-4 border-black bg-gray-900 text-white mt-auto">
                <div className="flex items-center flex-wrap gap-1.5 md:gap-2">
                    ...
                    {route ? route.split(',').map((segment, idx) => (
                        <span
                            key={idx}
                        ...
                        <span className="text-gray-400 text-xs">No route assigned</span>
                    )}
                </div>
            </div>
        </div>
